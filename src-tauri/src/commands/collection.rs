...
#[tauri::command]
pub async fn get_portfolio_history(state: State<'_, AppState>) -> Result<Vec<PortfolioDataPoint>, String> {
    let db = state.db.lock().map_err(|_| "Failed to lock db".to_string())?;
    
    // Get all unique dates from price history
    let mut dates_stmt = db.prepare(
        "SELECT DISTINCT date FROM price_history ORDER BY date ASC"
    ).map_err(|e| e.to_string())?;
    
    let dates: Vec<String> = dates_stmt.query_map([], |row| row.get(0))
        .map_err(|e| e.to_string())?
    ...
    for date in dates {
        ...
        let total_investment: f64 = db.query_row(
...
#[tauri::command]
pub async fn import_collection(state: State<'_, AppState>, csv_content: String) -> Result<String, String> {
    let db = state.db.lock().map_err(|_| "Failed to lock db".to_string())?;
    ...
    Ok(format!("Imported {} cards, skipped {} cards", imported, skipped))
}
